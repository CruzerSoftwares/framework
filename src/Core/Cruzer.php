...
class Cruzer {
/**
 * Define an autoload for classes
 * @author RN Kushwaha <rn.kushwaha022@gmail.com>
 * @since v 1.0.0 <date: 2nd Aug 2015>
 */

    public static function humanTiming ($time, $reseverse = false ) {
        if( $reseverse === true ){
      ...
      public static function _formatFlightTime( $time, $full=true ){
      ...
      public static function __status( $status, $formated = true, $statusFor='default' ){
         echo _status($status, $formated, $statusFor);
      }


      public static function _matchUrl( $urlToMatch='', $requestUrl ){
      ...
      public static function _pr( $arr=array(), $exit=false){
          echo "<pre>";
          print_r($arr);
          echo "<pre>";
          if($exit){
      ...
      public static function __( $string = null, $escape = true){
          if($escape === true){
              echo htmlspecialchars($string);
          } else{
              echo $string;
      ...
      public static function _form( $action='', $options = array(), $csrf = true ){
      ...
      public static function __form( $action='', $options = array(), $csrf = true ){
          echo _form( $action, $options, $csrf );
     }


      public static function _CSRF( $full_field = false ){
          $token = session_id();
      ...
      public static function _checkCSRF( $csrf='', $terminate = true, $terminate_msg = 'Invalid Token' ){
          if( $csrf == '' ){
      ...
      public static function _a( $title, $link, $options = array(), $escape = false ){
          if( $escape === false ){
              $anchor = '<a href="'.$link.'"';
      ...
      public static function __a( $title, $link, $options = array(), $escape = false ){
          echo _a($title, $link, $options , $escape );
     }

      public static function _css( $links, $defer = false, $version = false ){
          $css = '';
      ...
      public static function __css( $links, $defer = false, $version = false ){
          echo _css( $links, $defer, $version  );
     }

      public static function _url( $url, $absolute = true, array $options = array() ){
      ...
      public static function __url( $url, $absolute = true ){
          echo _url( $url, $absolute  );
     }


      public static function _js( $links, $defer = false, $version = false ){
          $js = '';
          if( is_array($links) && count($links) ){
              foreach ($links as $key => $value) {
                  if($version) $value = $value.'?v='.$version;
      ...
      public static function __js( $links, $defer = false, $version = false ){
          echo _js( $links  );
     }

      public static function _date( $datetime, $time = true, $format = null ){
          if( $datetime == null || $datetime=='' || $datetime=='0000-00-00' || $datetime=='0000-00-00 00:00:00'){
              return;
          }
          if($time === true){
            if( $format == null ){
              return date('M dS, Y h:i A',strtotime($datetime));
            } else{
              return date($format,strtotime($datetime));
            }
          } else{
              if( $format == null ){
                return date('M dS, Y',strtotime($datetime));
              } else{
                return date($format,strtotime($datetime));
              }
          }
      ...
      public static function __date( $datetime, $time = true, $format = null ){
          echo _date($datetime, $time, $format);
      }

      public static function _toDBDate( $datetime, $options = array() ){
      ...
      public static function _toDBDateFull( $datetime, $options = array() ){
          if( $datetime==null || $datetime=='0000-00-00' || $datetime=='0000-00-00 00:00:00'){
              return;
          }

          if( isset($options['format']) && $options['format']=='US'){
            $dateObj = DateTime::createFromFormat('m/d/Y H:i A', $datetime);
          }

          $dateObj = DateTime::createFromFormat('d/m/Y H:i A', $datetime);
          return $dateFormatted = $dateObj->format('Y-m-d H:i:s');
     }

      public static function _auth( $backend = true ){
      ...
      public static function _role( $backend = true ){
          $for     = ($backend === true ? 'backend' : 'frontend');
          $session = array();
      ...
      public static function _destroyAuthSession( $backend = true ){
          $for = ($backend === true ? 'backend' : 'frontend');
          $_SESSION[$for]['auth'] = null;
      }

      public static function _getAuthSession( $key = 'id', $backend = true ){
          $for = ($backend === true ? 'backend' : 'frontend');
          if( isset($_SESSION[$for]) && isset($_SESSION[$for]['auth']) && isset($_SESSION[$for]['auth']['id']) && $_SESSION[$for]['auth']['id']>0 ){
              if( $key == 'role') {
      ...
      public static function __getAuthSession( $key = 'id', $backend = true ){
          __(_getAuthSession($key, $backend));
      }

      public static function _flashSession( $backend = true ){
          $for     = ($backend === true ? 'backend' : 'frontend');
          $session = array();
      ...
      public static function __flashSession( $backend = true ){
          echo _flashSession($backend);
      }

      public static function _setFlash( $msg_type, $msg, $backend = true ){
          $for = ($backend === true ? 'backend' : 'frontend');
          $_SESSION[$for]['flash'][$msg_type] = $msg;
      }

      public static function _redirect( $url, $code = 200 ){
          if($url=='back'){
      ...
      public static function _upload( $tmp_name, $attachment, $folder = '' ){
    ...
    public static function _numberToCurrency($num) {
   ...
   public static function getQueryStrings($url='')
      ...
      public static function _requestData($var, $default = null, $method = 'post', $format = null) {
      ...
      public static function _getData($var, $default = null, $format = null) {
          return _requestData($var, $default, 'get', $format );
      }

      public static function _postData($var, $default = null, $format = null) {
          return _requestData($var, $default , 'post', $format );
      }

      public static function _mail(array $options = array()) {
  ...
  public static function _encodeString($string) {
      $result       = $string;
  ...
  public static function _decodeString($string) {
